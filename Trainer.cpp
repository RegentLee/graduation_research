...
*/
void Trainer::fit(SDNNWithUpdateOpenMP &model, vector<vector<int> > sample, string fp, int max_epoch, int batch_size) {
    int max_iters = sample.size();
    vector<vector<int> > data(sample.size(), vector<int>(sample[0].size()));
    ...
    for(int epoch = 1; epoch < 4; epoch++){
        ...
        for(int iters = 0; iters < max_iters; iters++){
            ...
            model.TrainWithUpdate(input, target);
    ...
    for(int epoch = 4; epoch < max_epoch; epoch++){
        //サンプルの順番を変える
        shuffle(range.begin(), range.end(), mt);
        for(int i = 0; i < sample.size(); i++){
            data[i].assign(sample[range[i]].begin(), sample[range[i]].end());
